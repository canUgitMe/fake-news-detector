...
const STEPS = [
  {
    num: 1,
    icon: (
      <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#00ff66" strokeWidth="1.4">
  ...
  return (
    <section className="hiw" id="how-it-works">
      <div className="hiw__container">
        ...
        {/*
          FIX: Number badge and card are now in the SAME column cell.
          This guarantees the number is always directly above its card
          on every screen size — no separate grids that can misalign.
        */}
        <div className="hiw__grid">
          {STEPS.map(function(step, i) {
            return (
              <div className="hiw__step" key={step.num}>
                {/* Number badge sits at the top of each step column */}
                <div className="hiw__num-wrap">
                  <div className="hiw__num">{step.num}</div>
                  {/* Connector line — shows between steps, not after last */}
                  {i < STEPS.length - 1 && (
                    <div className="hiw__line"></div>
                  )}
                </div>

                {/* Card directly below the number in the same column */}
